...
const InfoPanel = ({ lesson, ...rest }) => {
  ...
  const SetPrice = () => {
    if (lesson.price === null || lesson.price === 0) {
      return (
        <div className={styles.info}>бесплатно</div>
      )
    }
    else {
      ...
      return (
        <div className={styles.info}>{result}</div>
      )
    }
  }
  ...
  const SetAge = () => {
    if (lesson.startAge === null && lesson.endAge === null) {
      return (
        <div className={styles.info}>любой</div>
      )
    }
    else if (lesson.startAge !== null && lesson.endAge !== null) {
      return (
        <div className={styles.info}>{lesson.startAge}-{lesson.endAge} лет</div>
      )
    }
    else if (lesson.startAge !== null) {
      return (
        <div className={styles.info}>{lesson.startAge} лет</div>
      )
    }
    else {
      return (
        <div className={styles.info}>{lesson.endAge} лет</div>
      )
    }
  }
  ...
  const SetSex = () => {
    if (lesson.sex == 'female') {
      return (
        <div className={styles.info}>женский</div>
      )
    }
    else if (lesson.sex == 'male') {
      return (
        <div className={styles.info}>мужской</div>
      )
    }
    else {
      return (
  ...
  const SetWorkPeriods = () => {
    return (
      <div className={styles.info}>{lesson.isInNotSummer ? <img src={checkbox}></img> : <input className={styles.checkbox} disabled={true} />}
        &nbsp;сен-май&nbsp;&nbsp;
        {lesson.isInSummer ? <img src={checkbox}></img> : <input className={styles.checkbox} disabled={true} />}&nbsp;летом</div>
    )
  }
  ...
  return (
    <div className={styles.mainPanel}>
      <Sheet background='#EEEFEF' padding={sheetParams['padding']} width={sheetParams['width']} height={sheetParams['height']}>
        ...
        <div className={styles.infoRow}>
          ...
          {lesson.hasReception ? <div className={styles.info}>есть</div> : <div className={styles.info}>нет</div>}
        </div>
      </Sheet>
